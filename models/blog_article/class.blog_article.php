...
class blog_article extends Model
{
    ...
    /**
     * Gets a list of blog_article ids or count
     * @param   array   $a
     *                  - market_id
     *                  - where
     *                  - status
     ...
     * @return  array | int
     */
    public static function getList($a, $do_count = false)
    {
        // where
        $where = (is_array($a['where']))
            ? $a['where']
            : array($a['where']);
        ...
        if ($a['market_id']) {
            $where[] = "(
                blog_article.market_id = {$a['market_id']} OR
                blog_article.market_id = 0 OR
                blog_article.market_id IS NULL
            )";
        }
        ...
        if ($a['blog_category_id']) {
            $where[] = 'blog_category.id = '.$a['blog_category_id'];
        }
        ...
        // venue_id
        if ($a['venue_id']) {
            $where[] = "blog_article.venue_id = {$a['venue_id']}";
        }
        ...
        if ($a['tag']) {
            ...
            $first_join =  'LEFT JOIN blog_article
                                on blog_article.id = blog_article_tag.blog_article_id
                                and blog_article.active = 1';
            ...
            $tag_join = 'LEFT JOIN blog_article_tag
                            on blog_article.id = blog_article_tag.blog_article_id
        ...
        $sql = "SELECT blog_article_id FROM (
                    SELECT DISTINCT on (q.blog_article_id) blog_article_id, row FROM (
                        ...
                        {$first_join}
                        LEFT JOIN blog
                            on blog.id = blog_article.blog_id
                            and blog.active = 1
                        LEFT JOIN blog_category
                            on blog_category.id = blog_article.blog_category_id
                            and blog_category.active = 1
                        LEFT JOIN blog_media
                            on blog_media.blog_article_id = blog_article.id
                            and blog_media.active = 1
